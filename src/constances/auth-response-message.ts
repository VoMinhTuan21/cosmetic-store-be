// Success
export const SIGN_SUCCESS = 'SIGN_SUCCESS';
export const SIGN_UP_SUCCESS = 'SIGN_UP_SUCCESS';
export const SIGN_IN_SUCCESS = 'SIGN_IN_SUCCESS';
export const SEND_OTP_SUCCESS = 'SEND_OTP_SUCCESS';
export const CHECK_STATUS_WITH_SOCIAL_ACCOUNT_SUCCESS =
  'CHECK_STATUS_WITH_SOCIAL_ACCOUNT_SUCCESS';
export const LINK_ACCOUNT_SUCCESS = 'LINK_ACCOUNT_SUCCESS';

// Error
export const ERROR_ACCOUNT_ALREADY_LINK_TO_THIS_SOCIAL_MEDIA =
  'ACCOUNT_ALREADY_LINK_TO_THIS_SOCIAL_MEDIA';
export const ERROR_SIGN_UP = 'ERROR_SIGN_UP';
export const ERROR_EMAIL_HAS_BEEN_USED = 'ERROR_EMAIL_HAS_BEEN_USED';
export const ERROR_USER_NOT_EXIST = 'ERROR_USER_NOT_EXIST';
export const ERROR_PASSWORD_NOT_MATCH = 'ERROR_PASSWORD_NOT_MATCH';
export const ERROR_SIGN_IN = 'ERROR_SIGN_IN';
export const ERROR_CREATE_OTP = 'ERROR_CREATE_OTP';
export const ERROR_SEND_OTP = 'ERROR_SEND_OTP';
export const ERROR_VERIFY_OTP = 'ERROR_VERIFY_OTP';
export const ERROR_OTP_INCORRECT = 'ERROR_OTP_INCORRECT';
export const ERROR_SIGN_IN_WITH_SOCIAL_MEDIA =
  'ERROR_SIGN_IN_WITH_SOCIAL_MEDIA';
export const ERROR_CHECK_STATUS_WITH_SOCIAL_ACCOUNT =
  'ERROR_CHECK_STATUS_WITH_SOCIAL_ACCOUNT';
export const ERROR_ALREADY_LINKED_TO_ANOTHER_ACCOUNT =
  'ERROR_ALREADY_LINKED_TO_ANOTHER_ACCOUNT';
export const ERROR_LINK_ACCOUNT = 'ERROR_LINK_ACCOUNT';
